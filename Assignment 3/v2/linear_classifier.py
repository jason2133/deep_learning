...
def svm_loss_vectorized(W, X, y, reg):
  ...
  imp_margins = (margins > 0).type(X.dtype)
  imp_margins[range(num_train), y] -= (imp_margins.sum(axis=1))

  dW = X.T.matmul(imp_margins)
  dW /= (num_train)
  dW += (2 * reg * W)
...
def sample_batch(X, y, num_train, batch_size):
  ...
  batch_indexes = torch.randint(num_train, (batch_size, ))
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    dW[:, y[i]] -= ((expsum - each[y[i]])/expsum * X[i])
