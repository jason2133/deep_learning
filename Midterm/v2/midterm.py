...
class LinearRegress:
    ...
    def train_analytic(self, lam=0):
        ...
        self.X = torch.cat([ones_col, self.X], axis=1)

        beta = normal_equation(self.y, self.X, lam)
        yhat_train = self.X @ beta
    ...
    def predict(self, Xtest):
        ...
        Xtest = torch.cat([ones_col, Xtest], axis=1)

        yhat_test = Xtest @ self.beta
    ...
    def train_gd(self, niter, lr=1e-4):
        ...
        for t in range(niter):            
            ...
            yhat_train = X @ beta
            loss = ((self.y - yhat_train)**2).mean()
            beta = beta - lr * ((yhat_train - self.y) * X).mean()
    ...
    def train_adam(self, niter, lr=1e-3, beta1=0.9, beta2=0.999):
        ...
        for t in range(niter):
            #################################
            # TODO: fill out adam algorithm for the linear regression
            #################################
            # Replace "pass" statement with your code

            yhat_train = X @ beta
            w = ((yhat_train - self.y)**2).mean()

            dw = ((yhat_train - self.y) * X).mean()
            moment1 = beta1 * moment1 + (1 - beta1) * dw
            moment2 = beta2 * moment2 + (1 - beta2) * dw * dw
            moment1_unbias = moment1 / (1 - beta1 ** t)
            moment2_unbias = moment2 / (1 - beta2 ** t)
            w = w - lr * moment1_unbias / (moment2_unbias.sqrt() + 1e-7)
