...
def compute_distances_two_loops(x_train, x_test):
  ...
  # Euclidean distance 
  # between each element of the training set and each element of the test set

  # Images should be flattened and treated as vectors
  
  # 두 이미지 벡터 사이의 유클리디안 거리를 구한다.
  # for loop 이중을 활용해서.

  import numpy as np
  for i in range(num_train):
    for j in range(num_test):
      a = torch.flatten(x_train[i])
      b = torch.flatten(x_test[j])
      dist = np.square(a-b).sum()
      dists[i][j] += dist
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
    dists[i] = torch.sum((b - a[i]) ** 2, dim=1).t()
...
def compute_distances_no_loops(x_train, x_test):
  ...
  # dists = (torch.sum(a**2, dim=1).reshape(-1, 1) + torch.sum(b**2, dim=1).reshape(1, -1) - 2 * torch.mm(a, b.t()))
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
    a, b = torch.topk(dists[:, i], k, largest=False)
    a, b = y_train[b].unique(return_counts=True)
    y_pred[i] = a[b.argmax()]
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  # torch.chunk : Tensor 자르기 / 분리하기
  # chunk 함수는 tensor를 쪼개는 함수다. tensor를 몇개로 어떤 dimension으로 쪼갤지 설정해주고 사용하면 되나.
  ...
  for k_num in k_choices:
    ...
    for j in range(num_folds):
      folds_x_test = x_train_folds[j]
      folds_y_test = y_train_folds[j]

      folds_x_train = torch.cat([x_train_folds[k] for k in range(num_folds) if k != j])
      folds_y_train = torch.cat([y_train_folds[k] for k in range(num_folds) if k != j])

      classifier = KnnClassifier(folds_x_train, folds_y_train)
      accuracy = classifier.check_accuracy(folds_x_test, folds_y_test, k=k_num, quiet=True)
...
def knn_get_best_k(k_to_accuracies):
  ...
  for i, j in sorted(k_to_accuracies.items()):
    a.append(statistics.mean(j))
  
  best_k = sorted(k_to_accuracies.items())[(torch.argmax(torch.tensor(a))).item()][0]
