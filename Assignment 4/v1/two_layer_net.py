...
def nn_forward_pass(params, X):
    ...
    # clamp : 좀쇠로 고정시키다, 꽉 물다(잡다)
    # 해당 범주 값 이하 또는 이상이 되지 않도록 꽉 물어놓은 곳.
    # torch.clamp 함수 : 그 조건으로 적용

    # ReLU의 경우, clamp를 사용해서 0 미만의 값들을 그냥 0으로 다 만들어주는 시도를 해봄.
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    scores = scores - (scores.max(axis=1).values.view(-1, 1))
    scores = scores.exp()
...
def nn_get_search_params():
  ...
  # regularization_strengths = [3e-6, 1e-6, 5e-6, 7e-6]
  # learning_rate_decays = [0.90, 0.93, 0.95, 0.97]

  # 원래는 위 코드처럼 Hyperparameter 조합들의 경우의 수에 따라 일일이 테스트를 진행해야 하는데, 
  # 시간상의 이유로 인해 우선 하단 코드로 진행함.

  learning_rates = [1, 1.5]
  hidden_sizes = [2048, 4096]
  regularization_strengths=[1e-6, 5e-6]
  ...
  for i in learning_rates:
    for j in hidden_sizes:
      for k in regularization_strengths:
        for h in learning_rate_decays:
          model = TwoLayerNet(3*32*32, j, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)
          stats = model.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'], num_iters=3000, batch_size=1000, learning_rate=i, learning_rate_decay=h, reg=k, verbose=False)
