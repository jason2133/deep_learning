...
def count_tensor_elements(x):
  ...
  # Replace "pass" statement with your code
  num_elements = np.prod(list(x.size()))
...
def multiples_of_ten(start, stop):
  ...
  b = stop // 10
  if b>a:
    x=torch.tensor([(float(x)+1)*10 for x in range(b-a)], dtype=torch.float64)
  else:
...
def reverse_rows(x):
  ...
  # Replace "pass" statement with your code
  y = torch.zeros(M,N)
  idx = torch.flip(torch.arange(M),[0])
  y = x[idx,:]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop == True:
    ...
    ans = [x[i].mm(y[i]) for i in range(x.shape[0])]
    z = torch.stack(ans)
